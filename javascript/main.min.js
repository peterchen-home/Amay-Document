...
this.show()},initEvent:function(){this.inputButton.find("input").on("change",function(a){window.gotoPageDebounceTimer&&clearTimeout(window.gotoPageDebounceTimer);window.gotoPageDebounceTimer=setTimeout(function(){var b=parseInt(getRealPage(a.target.value));isNaN(b)||(b=Math.min(Math.max(b,1),bookConfig.totalPageCount),gotoPageFun(b,!1,"page input"))},10)});this.firstButton.onTap(function(){BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"first",getLanguage("btnFirstPage","First")]);
...
"next",getLanguage("btnNextPage","Next")]);return!1});this.lastButton.onTap(function(){BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"last",getLanguage("btnLastPage","Last")]);rightToLeft?firstPageFun("phone goto button"):lastPageFun("phone goto button");return!1});this.pagePrevButton.onTap(function(){rightToLeft?nextPageFun("phone goto button"):previousPageFun("phone goto button");BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"prev",getLanguage("btnPrePage","Previous")]);
return!1});this.pageNextButton.onTap(function(){rightToLeft?previousPageFun("phone goto button"):nextPageFun("phone goto button");BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"next",getLanguage("btnNextPage","Next")]);return!1});this.goButton.onTap(function(){var a=parseInt($("#currentPageIndexTextField").val().replace(/\D/g,""));gotoPageFun(Math.min(Math.max(a,1),bookConfig.totalPageCount),!1,"input");BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"goButton","goButton"]);
return!1}.bind(this));this.backwardButton.onTap(function(){rightToLeft?flipHistory.forward():flipHistory.backward();BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"backward",getLanguage("btnPageBack","Backward")]);return!1});this.forwardButton.onTap(function(){rightToLeft?flipHistory.backward():flipHistory.forward();BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"forward",getLanguage("btnPageForward","Forward")]);return!1})},destroy:function(){this.menu.remove();
global.phoneGotoPagePanel=null},showFirstOrLast:function(){if(bookConfig.phoneFlipShortcutButton&&(!(global.handSwipeHelp&&global.handSwipeHelp.visible||!global.handSwipeHelp&&bookConfig.showInstructionOnStart)||1!=BookInfo.getCurrentPageIndex())){if(bookConfig.RightToLeft){if(1==BookInfo.getCurrentPageIndex()){this.pagePrevButton.fadeIn(200);this.pageNextButton.hide();return}if(BookInfo.getCurrentPageIndex()==bookConfig.totalPageCount){this.pagePrevButton.hide();this.pageNextButton.fadeIn(200);return}}else{if(1==
BookInfo.getCurrentPageIndex()){this.pagePrevButton.hide();this.pageNextButton.fadeIn(200);return}if(BookInfo.getCurrentPageIndex()==bookConfig.totalPageCount){this.pagePrevButton.fadeIn(200);this.pageNextButton.hide();return}}this.pagePrevButton.fadeIn(200);this.pageNextButton.fadeIn(200)}}});
...
void 0!=$("#currentPageIndexTextField")[0]&&null!=$("#currentPageIndexTextField")[0]&&($("#currentPageIndexTextField").bind("keydown",function(a){if(13==a.keyCode){a=$("#currentPageIndexTextField").val();var b=parseInt(getRealPage(a));isNaN(b)||(b=Math.min(Math.max(b,1),bookConfig.totalPageCount),window.gotoPageDebounceTimer&&clearTimeout(window.gotoPageDebounceTimer),window.gotoPageDebounceTimer=setTimeout(function(){gotoPageFun(b,!1,"page input")},10))}}),$("#currentPageIndexTextField").bind("blur",
...
bookType==BookType.phone_book&&phoneBook&&phoneBook.setShow(!0),BookEvent.trigger("hideToolbar",[]))},showOrHide:function(){!0===this.visible?this.hide():this.show()},enterToShow:function(){this.show();var a,b,c;0!=(null!=(c=null==(a=global)?void 0:null==(b=a.userInfo)?void 0:b.c)?c:1)&&(this.enterOption=window.setTimeout(this.hide.bind(this),5E3))},addButton:function(a){switch(a){case "shopping":global.pageEditor&&global.pageEditor.setting&&parseBool(global.pageEditor.setting.shoppingCartHTML)&&
this.buttons.add(new PhoneShoppingCartButton);break;case "thumb":bookConfig.ThumbnailsButtonVisible&&this.buttons.add(new PhoneThumbnailButton);break;case "table":bookConfig.TableOfContentButtonVisible&&this.buttons.add(new PhoneTableOfContentButton);break;case "goto":this.buttons.add(new PhoneGotoPageButton);break;case "zoom":bookConfig.ZoomButtonVisible&&this.buttons.add(new PhoneZoomButton);break;case "bookmark":bookConfig.BookMarkButtonVisible&&this.buttons.add(new PhoneBookmarkButton);break;
case "number":bookConfig.PhoneButtonVisible&&this.buttons.add(new PhoneNumberButton);break;case "sound":bookConfig.BackgroundSoundButtonVisible&&this.buttons.add(new PhoneBackgroundSoundButton);break;case "search":bookConfig.SearchButtonVisible&&this.buttons.add(new PhoneSearchButton);break;case "print":bookConfig.PrintButtonVisible&&this.buttons.add(new PhonePrintButton);break;case "message":bookConfig.MessageButtonVisible&&this.buttons.add(new PhoneMessageButton);break;case "annotation":bookConfig.AnnotationButtonVisible&&
this.buttons.add(new PhoneAnnotationButton);break;case "wechat":bookConfig.WeChatShareButtonVisible&&this.buttons.add(new PhoneWeChatShareButton);break;case "btnshare":bookConfig.ShareButtonVisible&&this.buttons.add(new PhoneShareButton);break;case "download":bookConfig.DownloadButtonVisible&&this.buttons.add(new PhoneDownloadButton);break;case "language":1<language.length&&this.buttons.add(new PhoneLanguageButton);break;case "home":bookConfig.HomeButtonVisible&&this.buttons.add(new PhoneHomeButton);
break;case "autoflip":bookConfig.AutoPlayButtonVisible&&this.buttons.add(new PhoneAutoPlayButton);break;case "email":bookConfig.EmailButtonVisible&&this.buttons.add(new PhoneEmailButton);break;case "select":bookConfig.SelectTextButtonVisible&&this.buttons.add(new PhoneSelectTextButton);break;case "about":bookConfig.aboutButtonVisible&&this.buttons.add(new PhoneAboutButton);break;case "report":bookConfig.EnableReportButton&&this.buttons.add(new PhoneReportButton);break;case "doublesinglepage":bookConfig.DoubleSinglePageButtonVisible&&
this.buttons.add(new DoubleSinglePageButton);break;case "videogallery":bookConfig.VideoButtonVisible&&this.buttons.add(new PhoneVideoButton);break;case "pageflip":bookConfig.phoneFlipShortcutSwitchButtonVisible&&"brief"!=global.templeteName&&this.buttons.add(new PhonePageFlipButton);break;case "fullscreen":!bookConfig.FullscreenButtonVisible||!isAndroid()&&"brief"!=global.templeteName||NeatStyle.isNeat()||this.buttons.add(new PhoneFullscreenButton);break;case "neat":NeatStyle.isNeat()&&this.buttons.add(new PhoneNeatButton);
